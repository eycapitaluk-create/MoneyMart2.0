...
create or replace function public.prevent_client_premium_profile_writes()
returns trigger
language plpgsql
security definer
...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    then
      raise exception 'premium profile fields are service-role only';
    end if;
    return new;
  end if;
...
drop trigger if exists trg_prevent_client_premium_profile_writes on public.user_profiles;
create trigger trg_prevent_client_premium_profile_writes
before insert or update on public.user_profiles
for each row execute function public.prevent_client_premium_profile_writes();
