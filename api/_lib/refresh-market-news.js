...
export const refreshMarketNewsManualFeed = async () => {
  ...
  try {
    await replaceNewsManualBucketRows(adminClient, buckets, rows, { batchUpdatedAt: now })
  } catch (error) {
    return { status: 500, body: { ok: false, error: error?.message || 'news_manual replacement failed' } }
  }
