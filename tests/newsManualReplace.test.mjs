...
test('inserts a timestamped batch before deleting only older rows in target buckets', async () => {
  ...
  assert.deepEqual(ops, ['from', 'insert', 'select', 'from', 'delete', 'in', 'or'])
  ...
  const staleRowsFilter = client.calls.find((call) => call.op === 'or')
  assert.equal(staleRowsFilter.filter, `updated_at.is.null,updated_at.lt.${insertCall.rows[0].updated_at}`)
